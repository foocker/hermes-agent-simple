...
class Platform(Enum):
    ...
    Hermes Simple keeps enum compatibility with the upstream gateway so shared
    session/config code can be reused, but only Feishu and WeCom adapters are
    instantiated by gateway.run.
    """
    LOCAL = "local"
    TELEGRAM = "telegram"
    DISCORD = "discord"
...
_PLATFORM_CONNECTED_CHECKERS: dict[Platform, Callable[[PlatformConfig], bool]] = {
    Platform.FEISHU: lambda cfg: bool(
        cfg.extra.get("app_id")
        or cfg.extra.get("app_secret")
        or os.getenv("FEISHU_APP_ID")
        or os.getenv("FEISHU_APP_SECRET")
    ),
    Platform.WECOM: lambda cfg: bool(
        cfg.extra.get("bot_id")
        or cfg.extra.get("secret")
        or os.getenv("WECOM_BOT_ID")
        or os.getenv("WECOM_SECRET")
    ),
    Platform.WECOM_CALLBACK: lambda cfg: True,
}
