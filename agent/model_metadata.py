...
# Only these are stripped. Local "model:tag" colons are preserved so the full
# model name reaches cache lookups and server queries.
...
def _strip_provider_prefix(model: str) -> str:
    ...
    ``"model:7b"``      → ``"model:7b"``     (unchanged — local model:tag)
    """
    ...
    if prefix_lower in _PROVIDER_PREFIXES:
        # Don't strip if suffix looks like a local model tag (e.g. "7b", "latest", "q4_0")
...
# Thin fallback defaults. Keep this limited to OpenAI/GPT-family models.
# OpenRouter uses live API metadata; custom/local endpoints rely on endpoint
# metadata and local probes.
DEFAULT_CONTEXT_LENGTHS = {
    # OpenAI — GPT-5 family (most have 400k; specific overrides first)
    # Source: https://developers.openai.com/api/docs/models
    # GPT-5.5 (launched Apr 23 2026) is 1.05M on the direct OpenAI API and
    # ChatGPT Codex OAuth caps it at 272K; both paths resolve via their own
    # branches (_resolve_codex_oauth_context_length or direct OpenAI fallback).
...
_OFFICIAL_OPENAI_HOSTS = ("api.openai.com", "chatgpt.com")


def _is_official_openai_base_url(base_url: str) -> bool:
    return any(base_url_host_matches(base_url, host) for host in _OFFICIAL_OPENAI_HOSTS)


def _is_known_provider_base_url(base_url: str) -> bool:
    return _is_openrouter_base_url(base_url) or _is_official_openai_base_url(base_url)
...
def detect_local_server_type(base_url: str, api_key: str = "") -> Optional[str]:
    ...
    Returns one of: "ollama", "vllm", "llamacpp", or None.
    """
    import httpx
...
def _model_id_matches(candidate_id: str, lookup_model: str) -> bool:
    ...
    Supports two forms:
    - Exact match:  "model-slug" == "model-slug"
    - Slug match:   "publisher/model-slug" matches "model-slug"
                    (the part after the last "/" equals lookup_model)

    Some local servers store models as "publisher/slug" while users configure
    only the slug after the "local:" prefix.
    """
...
def _query_local_context_length(model: str, base_url: str, api_key: str = "") -> Optional[int]:
    ...
    # Local "model:tag" colons are intentionally preserved.
    ...
    try:
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            # OpenAI-compatible local servers: try /v1/models/{model}
...
def get_model_context_length(
    ...
    4. ChatGPT Codex OAuth metadata/fallback
    5. OpenRouter live API metadata
    6. Thin OpenAI/GPT-family hardcoded defaults
    7. Default fallback (probe-down)
    """
    ...
    # local servers actually know about.  Local "model:tag" colons are preserved.
    model = _strip_provider_prefix(model)
    ...
    if base_url:
        ...
        if cached is not None:
            ...
            # resolved gpt-5.x to the direct-API value (e.g. 1.05M) and
            # persisted it. Codex OAuth caps at 272K for every slug, so any
            # cached Codex entry at or above 400K is stale. Drop it and fall
            # through to the live /models probe below.
    ...
    # Official OpenAI/OpenRouter endpoints skip this path and use their
    # provider-specific handling below.
    ...
    # 6. Hardcoded OpenAI/GPT-family defaults
    # Only check `default_model in model` (is the key a substring of the input).
    # The reverse (`model in default_model`) causes shorter names like
    # shorter names to incorrectly match longer, more specific model IDs.
