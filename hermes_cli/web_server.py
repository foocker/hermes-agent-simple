...
# Device-code auth for Codex can run in-browser. For unconnected providers we
# return the canonical ``hermes auth add <provider>`` command so the dashboard
...
# right UI: ``device_code`` = show code + verification URL + poll.
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    """Initiate a device-code flow for OpenAI Codex.

    Spawns a background poller. Returns the user-facing display fields so the
    UI can render the verification page link + user code.
    """
    if provider_id == "openai-codex":
