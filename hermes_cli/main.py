...
def _has_any_provider_configured() -> bool:
    ...
    # Check for Codex OAuth credentials
...
def select_provider_and_model(args=None):
    ...
    if active in {"openai", "openrouter"} and get_env_value("OPENAI_BASE_URL"):
        active = "custom"
    ...
    if selected_provider == "openai":
        _model_flow_openai(config, current_model)
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Hermes only falls back to a lightweight GPT backend (OpenRouter")
        print("  or Vercel AI Gateway) if the main model is unavailable. Override a")
...
def _model_flow_openai(config, current_model=""):
    """OpenAI GPT provider: ensure API key, then pick a GPT model."""
    from hermes_cli.auth import (
    ...
    if not api_key:
        ...
        try:
            ...
            key = getpass.getpass("OpenAI API key (or Enter to cancel): ").strip()
        except (KeyboardInterrupt, EOFError):
            print()
            return
        if not key:
    ...
    model_list = provider_model_ids("openai", force_refresh=True) or [
        "gpt-5.4",
        "gpt-5.4-mini",
        "gpt-5-mini",
        "gpt-5.3-codex",
    ...
    if selected:
        ...
        save_config(cfg)
        deactivate_provider()
        print(f"Default model set to: {selected} (via OpenAI GPT)")
    else:
        print("No change.")
...
def main():
    ...
    logout_parser.add_argument(
        "--provider",
        choices=["openai", "openrouter", "ai-gateway", "openai-codex"],
        default=None,
    ...
    auth_add.add_argument(
        "provider",
        help="Provider id: openai, openrouter, ai-gateway, openai-codex, or a saved custom endpoint",
    )
    auth_add.add_argument(
        "--type",
        dest="auth_type",
...
""",
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP client server connections",
        description=(
            "Manage MCP server connections used by Hermes as an MCP client.\n\n"
