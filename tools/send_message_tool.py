"""Send Message Tool -- Feishu/WeCom messaging via platform APIs.

Hermes Simple keeps this tool focused on Feishu and Enterprise WeChat. It still
supports listing available targets and resolving human-friendly channel names.
"""
...
SEND_MESSAGE_SCHEMA = {
    "name": "send_message",
    "description": (
        "Send a message to Feishu or WeCom, or list available targets.\n\n"
        "IMPORTANT: When the user asks to send to a specific channel or person "
        "(not just a bare platform name), call send_message(action='list') FIRST to see "
        "available targets, then send to the correct one.\n"
        "If the user just says a platform name like 'send to feishu' or 'send to wecom', "
        "send directly to the configured home channel without listing first."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "target": {
                "type": "string",
                "description": "Delivery target. Format: 'feishu' or 'wecom' for home channel, 'platform:#channel-name', 'feishu:chat_id', 'feishu:chat_id:thread_id', or 'wecom:chat_id'."
            },
            "message": {
                "type": "string",
...
def _handle_send(args):
    ...
    if platform_name not in {"feishu", "wecom", "wecom_callback"}:
        return tool_error("Hermes Simple send_message only supports feishu, wecom, and wecom_callback")
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None):
    ...
    _MAX_LENGTHS[Platform.WECOM] = 4000
    _MAX_LENGTHS[Platform.WECOM_CALLBACK] = 4000
    ...
    if media_files and platform not in (Platform.FEISHU,):
        return {"error": f"MEDIA attachments are only supported for feishu in Hermes Simple; target was {platform.value}"}
    ...
    for chunk in chunks:
        ...
        elif platform in (Platform.WECOM, Platform.WECOM_CALLBACK):
            result = await _send_wecom(pconfig.extra, chat_id, chunk)
        else:
            result = {"error": f"Hermes Simple send_message only supports feishu and wecom, not {platform.value}"}
